...
def main(basedir, field_names, exclude_disk, ymax, mode):
    ...
    for j, grain in enumerate(field_names):
        ax[0].plot(times/1e3, dust_hot[j], linestyle=styles[j], linewidth=linewidth, color=color_hot)
    ax[0].tick_params(axis='both', which="both", labelsize=15, top=True, right=True)
    ax[0].set_yscale('log')
    ax[0].set_ylabel(r"$m_{dust}~[M_\odot]$", fontsize=20)
    ax[0].set_xlabel(r"$Time~[Myr]$", fontsize=20)
    ax[0].set_xlim(np.amin(times/1e3), np.amax(times/1e3))
    ax[0].set_ylim(ymin, ymax)

    for j, grain in enumerate(field_names):
        ax[1].plot(times/1e3, dust_mixed[j], linestyle=styles[j], linewidth=linewidth, color=color_mixed)
    ...
    for j, grain in enumerate(field_names):
        ax[2].plot(times/1e3, dust_cool[j], linestyle=styles[j], linewidth=linewidth, color=color_cool)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(basedir, "png", "dust_evolution.png"), dpi=300)
    plt.close()
