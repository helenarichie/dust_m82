...
disk_masses = np.zeros((4, len(time_output), 3))
    ...
    with open(os.path.join(csvdir, f"{field}.csv")) as f:
        for line in f:
            ...
            for i, bin in enumerate(line):
                ...
                while("" in bin):
                    bin.remove("")
                if float(bin[0]) == time_output[tmax_i]:
                    disk_masses[j][tmax_i] = bin[1:4] # 1:4 = hot, mixed, cold (excludes 0, which is time)

                    if tmax_i < (len(time_output)-1):
                        tmax_i += 1
...
for i, field in enumerate(fields):
    plt.semilogy(time_output/1e3, mass_out_hot[i]/disk_masses[i,0,0], linewidth=linewidth-1, linestyle=styles[i], c=color_hot, zorder=0)
    plt.semilogy(time_output/1e3, mass_out_mixed[i]/disk_masses[i,0,1], linewidth=linewidth-1, linestyle=styles[i], c=color_mixed, zorder=0)
    plt.semilogy(time_output/1e3, mass_out_cool[i]/disk_masses[i,0,2], linewidth=linewidth-1, linestyle=styles[i], c=color_cool, zorder=0)
plt.legend(fontsize=15)
# plt.xlim(xmin/1e3, xmax/1e3)
plt.tick_params(axis='both', which='both', direction='in', color='black', top=1, right=1, length=9, width=2, reset=True)
# plt.xticks(np.linspace(0, xmax/1e3, 5).round(1))
# plt.ticklabel_format(axis='y', style='sci', scilimits=(0,0))
plt.xlabel("Time [Myr]", fontsize=fontsize+3)
plt.ylabel(r"$m_{out}/m_{disk}$", fontsize=fontsize+5)
plt.tight_layout()
plt.savefig(pngdir + f"fluxes_{name}_dust_norm_init.png", dpi=300, bbox_inches="tight")
plt.close()
