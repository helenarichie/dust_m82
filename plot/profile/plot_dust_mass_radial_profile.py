...
def main(basedir, outdir, fnum, simulation, weight, tail, r_mask, mode, field_names, ymin, ymax):
    ...
    bin_tot, _, dust_hot, _, _, _, _, _, _, _ = read_radial_profile(basedir, fnum, simulation, "hot", weight, tail)
    bin_tot, _, dust_cool, _, _, _, _, _, _, _ = read_radial_profile(basedir ,fnum, simulation, "cold", weight, tail)
    ...
    for i, field in enumerate(field_names):
        ax[0].semilogy(bin_tot[bin_tot>r_mask], dust_cool[i][bin_tot>r_mask], linewidth=linewidth, linestyle=styles[i], color=color_cool)
        ax[1].semilogy(bin_tot[bin_tot>r_mask], dust_hot[i][bin_tot>r_mask], linewidth=linewidth, linestyle=styles[i], color=color_hot)
    ...
    ax[0].set_ylabel(r"$m_{dust}$ $[M_\odot]$")
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, f"{simulation}_dust_profile{tail}.png"), dpi=300)
    plt.close()
